...
2. **Automatic polling with auto-transition**: Start automatic status checking:
   ```
   /loop 1m /check-approval <approvalId> next:/spec-design
   ```
   ...
   - **approved**: Cleanup is performed automatically, loop stops, and check-approval automatically invokes `/spec-design`
...
3. **Handle needs-revision** (if loop stopped with revision request):
   ...
   - Submit a NEW approval request and start a new `/loop 1m /check-approval <newApprovalId> next:/spec-design`
