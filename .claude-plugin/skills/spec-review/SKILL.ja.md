---
name: spec-review
description: "ユーザー承認をリクエストする前にスペックドキュメントをセルフレビューする。レビュー詳細をメインコンテキストの外に保つために Agent ツールでサブエージェントとして起動するよう設計されている。スペックドキュメント（requirements.md、design.md、tasks.md）の作成・更新後、承認リクエスト前に自動的に使用する。トリガー: スペックドキュメント作成時、承認リクエスト前、'review spec'、'check spec quality'。"
---
