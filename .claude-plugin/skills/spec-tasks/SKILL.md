...
Agent({
  ...
  prompt: "You are a spec document reviewer. Review the document (do NOT modify the file) at:
    ...
    26. DESIGN_OBLIGATION_COVERAGE: Beyond DES/MOD components (covered by check 3), scan design.md's PROSE for standalone execution obligations — statements that mandate a CI job, workflow, build gate, or test execution outside any DES/MOD section (typical markers: 'CI の…ジョブで…実行する', '実行対象である', 'ゲートとして担保する', items in Phase Deliverables, notes under Excluded Test Environments). Every such obligation must be realized by some task: its File: list or _Prompt.Task must name the workflow file / artifact that implements it. An obligation with no implementing task → FAIL rule_id OBLIGATION_unassigned, citing the design.md line; fix_hint: 'Add a task (or extend an existing CI task's File: list) that implements the obligation — e.g., a cross-platform UT job declared in Excluded Test Environments notes needs a task touching .github/workflows/.'

    Reporting: for EC1/EC2/EC3 issues and for check 26, include fields rule_id, location, message, fix_hint.
