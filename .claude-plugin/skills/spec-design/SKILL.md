...
>
> Placement caveat: each trait declared here must also satisfy Module Boundaries authoring rule 7 — the trait's layer is decided by the types in its signatures. A Clock/Store trait whose signature mentions domain types is a domain-layer port (only its production impl goes in an outer layer); placing it in the lowest layer contradicts the dependency table and fails Step B check 18.
...
4. Place cross-cutting concerns (error, config, etc.) in the lowest layer and allow references from all layers — but see rule 7 for the limit of this rule
5. If there are no layer definitions (small projects, etc.), the section itself may be omitted
6. **Composition root**: startup wiring (`main` / bootstrap) necessarily references both entry adapters and every lower layer, so it can never live inside an ordinary layer (entry adapters mutually forbid each other). Give it its own top row in the layer table and the dependency table (e.g., `bootstrap | src/bootstrap.rs, src/main.rs | Allowed: all layers`) instead of leaving it unassigned
7. **A trait's layer is decided by the types in its signatures**: an injection boundary declared in Architecture for Testability (Clock, RNG, Store, ...) whose signatures mention domain types belongs in the domain layer as a port, with only the production implementation in an outer layer. Rule 4 applies only to types that reference nothing above them (pure error/config types). Applying rule 4 to a trait whose signature returns an upper-layer type creates a table-vs-signature contradiction that Step B check 18 (LAYER_DEPENDENCY_CLOSURE) flags
8. **Full coverage**: the layer table and the shared-type table must jointly assign a directory to every DES-N component and every MOD-N type. A component or type with no home forces implementers and reviewers to invent one at implementation time
...
Agent({
  ...
  prompt: "You are a spec document reviewer. Review the document (do NOT modify the file) at:
    ...
    18. LAYER_DEPENDENCY_CLOSURE: Applies when a Dependency Direction Rules (依存方向ルール) table exists. Derive and verify the full dependency graph the document implies — the contradiction this catches only appears when three separate sections (layer table, shared-type placement, dependency table) are joined, so never check them in isolation:
        a. Assign every '### DES-N:' a layer from its declared placement (directory in the layer table, Mock points table, or an explicit file path anywhere in the document). A DES-N whose placement matches no defined layer, or that cannot be assigned at all → FAIL rule_id LDC_unassignedComponent; fix_hint: 'Add the component's directory to the layer table (define a composition-root/bootstrap row if needed) or state its placement explicitly.'
        b. Assign every custom type ('### MOD-N:' and every type named in the shared-type table) a home layer/crate from the 共有型定義 table or its MOD section. A type with no placement → FAIL rule_id LDC_unassignedType.
        c. For every DES-N, collect its outgoing edges from BOTH the 'Dependencies:' field AND every custom type appearing in 'Interfaces:' signatures (parameters, return types, and inner types of generics such as Result<T, E> / Vec<T> / Option<T>). For each edge, verify (component's layer → target's layer) is Allowed by the dependency table. Same-layer edges and edges into a layer/crate the document declares as the shared bottom layer are allowed. Any edge the table marks Forbidden → FAIL rule_id LDC_forbiddenEdge, citing the DES-N, the type, and the violated table row. (Example: a Clock trait placed in infra whose signature returns a domain-layer Timestamp, while the table forbids infra → domain.)
        d. A component whose Dependencies span two layers that mutually forbid each other (typical for a composition root / bootstrap that wires both entry adapters) can never be legally placed in any defined layer; the layer table must contain an explicit top row for it (allowed to reference all layers). Missing → FAIL rule_id LDC_missingCompositionRoot.
    19. SNIPPET_LIBRARY_COMPATIBILITY: For each MOD-N code snippet, verify the snippet is implementable VERBATIM under the libraries the same document mandates (採用技術とバージョン table, MOD prose such as 'thiserror で定義'). Reason from the mandated library's actual semantics; when uncertain, verify with a scratch probe (throwaway compile) instead of assuming. Known trap patterns (non-exhaustive):
        - Rust thiserror: a variant field literally named `source` (or `backtrace`) whose type does not implement the required trait makes #[derive(thiserror::Error)] fail to compile as written; the snippet must note the `r#source` raw-identifier escape or use a different field name.
        - Rust serde: `#[serde(rename_all = "...", tag = "...")]` on an enum renames VARIANTS only; struct-variant FIELDS keep their original case unless `rename_all_fields` is also given. A snippet with multi-word struct-variant fields but no `rename_all_fields`, in a document declaring a camelCase wire contract, does not satisfy its own contract.
        Violation → FAIL rule_id SNIPPET_libIncompat; fix_hint: 'Amend the snippet (or add an explicit note next to it) so that verbatim transcription under the mandated library compiles and satisfies the declared wire contract.'
    20. PROSE_IDENTIFIER_XREF: Every variant, field, type, or method name referenced in prose (backticked or plain) must exist in a snippet, a MOD/DES definition, or a mandated library API within this document. Near-miss mismatches (e.g., prose says `Rejected` but the enum defines `Reject`) → FAIL rule_id PROSE_identifierMismatch, reporting both spellings and their locations.
    21. PRINCIPLE_INTERFACE_CONSISTENCY: Collect the declared design principles (e.g., 型共有の原則 'domain types and API types are separated; convert at the boundary') and verify no DES-N 'Interfaces:' signature contradicts them (e.g., a domain-layer service taking an API request DTO directly). A deliberate exception is acceptable ONLY when the exception is written next to the principle or inside the DES section. Unstated contradiction → FAIL rule_id PRINCIPLE_interfaceConflict; fix_hint: 'Either change the interface to honor the principle, or write the exception into the principle so implementers and reviewers have a single answer instead of re-litigating it at review time.'

    Reporting: for EC1/EC2/EC3 issues and for checks 18-21, include fields rule_id, location, message, fix_hint.
