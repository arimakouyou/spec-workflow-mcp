...
Agent({
  ...
  prompt: `あなたは実装レビューの専門家です。Phase ${phaseNumber} で変更されたコードを、仕様書の観点からレビューしてください。
    ...
    参照ファイル:
    - ${projectPath}/.spec-workflow/specs/${specName}/requirements.md
    - ${projectPath}/.spec-workflow/specs/${specName}/design.md
    - ${projectPath}/.spec-workflow/specs/${specName}/tasks.md
...
Agent({
  ...
  prompt: `あなたはセキュリティレビューの専門家（認証・認可・データ保護）です。Phase ${phaseNumber} で変更されたコードをレビューしてください。
...
Agent({
  ...
  prompt: `あなたはセキュリティレビューの専門家（OWASP・脆弱性）です。Phase ${phaseNumber} で変更されたコードをレビューしてください。

    プロジェクトパス: ${projectPath}
    変更ファイル: ${changedFiles}

    レビュー観点:
    OWASP TOP 10 (2021) の各項目について確認:
    - A01: アクセス制御の不備
    - A02: 暗号化の失敗
    ...
    追加確認:
    - 使用しているクレートのバージョンに既知の CVE がないか（Cargo.toml を確認）
    - 入力バリデーションの漏れ
    - エラーメッセージによる情報漏洩
...
Agent({
  ...
  prompt: `あなたはパフォーマンスレビューの専門家です。Phase ${phaseNumber} で変更されたコードをレビューしてください。
...
Agent({
  ...
  prompt: `あなたは品質・保守性レビューの専門家です。Phase ${phaseNumber} で変更されたコードをレビューしてください。
