...
| `task-types.md` | TT1〜TT6 | 6 | Spec タスク類型と事前調査カテゴリ（Phase 0.5 `/spec-investigate` の入力。TT2: 5 類型と必須 evidence category、TT3: category 定義、TT4: ユーザー上書き、TT5: legacy 例外） |
| `evidence-coverage.md` | EC1〜EC6 | 6 | Evidence 引用記法と仕様書本体コード引用行数上限（spec-requirements/design/test-design/tasks の Step B check が参照。EC1: 引用形式、EC3: 行数ガード、EC5: legacy 例外） |
...
| `task-types.md` | Spec タスク類型（TT1-TT6: 5 類型定義 + 必須 evidence category + 上書き設定 + legacy 例外）。`/spec-request-spec` で宣言、`/spec-investigate` で消費 |
| `evidence-coverage.md` | Evidence 引用規則と本体コード行数上限（EC1-EC6: EV-ID 形式 + 引用整合 + コード budget + legacy 例外）。spec-requirements / spec-design / spec-test-design / spec-tasks の Step B check が参照 |
