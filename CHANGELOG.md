...
- **grilling の JEV 回答**: 各ラウンドの問いはまず JEV に答えさせ、閾値(noul ≥0.85 / ≤0.15、choice は confidence ≥0.6 かつ差 ≥0.2、score は最大確率 ≥0.5)を満たさなかった問いだけをユーザーに尋ねる。判定は `scripts/spec-grill-jev.sh` が行う(watany-dev/jev-playground の gril-jev skill を参考にした)。jevcli が無いか失敗したときは全問をユーザーに尋ねる
...
- **参照は行番号ではなく安定したアンカーで書く** - `rules/doc-crossref.md` に「Reference Form」節を追加。コミットされる文書（spec 文書・`_Prompt`・申し送り・タスクログ・ADR・refactor-backlog・コードコメント・コミットメッセージ）では `design.md:923` のような `file:line` 参照を禁止し、ID（REQ / DES / MOD / IT / ST / API）・見出し・表の行キー・grep 可能な逐語句・シンボルで指す。spec-design / spec-test-design / spec-tasks の Rules、parallel-worker のログ規則、task-log-format、review-worker の A 観点（`file:line` は Minor として auto-fix）に反映（specrail approval-gate で design.md の改訂後に `design.md:923` の 13 箇所が無関係な見出しを指し続けた件の再発防止）
...
- **confirm-phase-progression.sh の過剰ブロックを修正** - Phase/Wave 進行宣言の検出対象を「今ターンの最終 assistant 発言」（Stop フック入力の `last_assistant_message`）に限定。従来は transcript の `tail -n 200` を丸ごと grep しており、tool_result・過去の発言・ユーザー発話に含まれる文字列で誤ブロックしていた（実測: 本リポジトリの 12 transcript で 4 件 → 0 件、specrail の実ブロック 57 件 → 0 件、specrail 全 885 assistant 発言の総当たりでも 0 件）。あわせて以下を修正:
  ...
  - ダッシュボード承認（「承認した」「承認済み」等）を同意として認識。フェーズ遷移のゲートは承認ワークフローであり（`spec-design/SKILL.md` ・ `spec-requirements/SKILL.md` の「`check-approval` fetches status once」節）、承認済みと伝えられた直後の Phase 遷移をブロックするのは仕様と矛盾していた。否定形（「まだ承認されていない」）は同意として扱わない
...
- **Plugin templates** (`.claude-plugin/templates/`) - Standard-form templates for authoring new plugin components:
  - `SKILL.md.template` - Skill frontmatter and body structure with description-writing guidelines
  - `rule.md.template` - Rule structure with ID prefix, enforcement level, and promote/demote heuristics
...
- **Plugin hook cleanup** - Reduced `PreToolUse` timeouts and consolidated `PostToolUse` hooks to improve iteration UX:
  - `format-check-guard.sh` timeout reduced from 120s to 30s
  - `post-edit-check.sh` + `post-edit-markdownlint.sh` merged into a unified `post-edit.sh` (20s timeout) that dispatches rustfmt / markdownlint / prettier / dotnet format by file extension
- **`security-audit-guard.sh` plugin hook** - Rewrote the unconditional 180s vulnerability scan into a diff-aware fail-close hook:
  - Runs `cargo audit` / `npm audit` / `dotnet list package --vulnerable` only when a `git commit` stages manifest files with dependency-section changes or lockfile files of the affected ecosystem (Rust / Node / .NET)
  - Each audit command has a 120s internal timeout; commits are blocked (exit 2) on both vulnerability detection and audit timeout (fail-close)
