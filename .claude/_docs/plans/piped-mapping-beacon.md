...
1. **Before the first rework**: Initialize `diagnostic_history` with the marker string `"(First rework — no prior attempts)"`
2. **After each rework attempt**: Extract from parallel-worker's completion report:
   - The `diagnosis` summary fields: `root_cause`, `responsible_files` (list), `approach`
