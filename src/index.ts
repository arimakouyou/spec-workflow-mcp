...
For more information, visit: https://github.com/arimakouyou/spec-workflow-mcp
`);
}
...
async function main() {
  try {
    ...
    if (error.message.includes('ENOENT') || error.message.includes('path') || error.message.includes('directory')) {
      ...
      console.error('- For Claude CLI users, ensure you used: claude mcp add spec-workflow npx -y @arimakouyou/spec-workflow-mcp@latest -- /path/to/your/project');
